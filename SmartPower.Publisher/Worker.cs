...
namespace SmartPower.Publisher
{
    public class Worker : BackgroundService
    {
        ...
        async Task ControlArduino(CancellationToken stoppingToken)
        {
            ...
            _fridge.Loop(temp1.Value, temp2.Value);

            if (!_arduino.TurnRelayOnOff(0, _fridge.CompressorOn)) { OnSerialError(); return; }
            if (!_arduino.TurnRelayOnOff(1, _fridge.EvapFanOn)) { OnSerialError(); return; }
            if (!_arduino.TurnRelayOnOff(2, _fridge.DefrostOn)) { OnSerialError(); return; }
            if (!_arduino.TurnRelayOnOff(3, _fridge.LightOn)) { OnSerialError(); return; }
        }
